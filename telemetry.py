...
def draw_corners(ax, session):
    ...
    for index, (_, corner) in enumerate(corners.iterrows()):
        ...
        ax.annotate(f"{int(corner['Number'])}",
                    ...
                    color=style.MUTED, fontsize=8, zorder=5)
...
def plot_gear_map(year, race, driver, lap_number=None):
    ...
    collection = LineCollection(segments, cmap='viridis', linewidth=6,
                                capstyle='round', zorder=2)
...
def plot_gap_to_leader(year, race, drivers=None):
...
def animate_head_to_head(year, race, driver_a, driver_b, frames=500):
    ...
    fig, ax = plt.subplots(figsize=(11, 9))
    ax.plot(tel_a['X'], tel_a['Y'], linewidth=11, color=style.TRACK,
            solid_capstyle='round', zorder=1)
    draw_corners(ax, session)
    ...
    car_b, = ax.plot([], [], 'o', markersize=15, color=style.DRIVER_B,
                     markeredgecolor=style.BACKGROUND, markeredgewidth=2,
                     zorder=4)
    car_a, = ax.plot([], [], 'o', markersize=15, color=style.DRIVER_A,
                     markeredgecolor=style.BACKGROUND, markeredgewidth=2,
                     alpha=0.85, zorder=5)

    clock = ax.text(0.02, 0.97, '', transform=ax.transAxes,
                    color=style.TEXT, fontsize=13, va='top',
                    family='monospace', zorder=6)
    line_a = ax.text(0.02, 0.93, '', transform=ax.transAxes,
                     color=style.DRIVER_A, fontsize=12, va='top',
                     family='monospace', zorder=6)
    line_b = ax.text(0.02, 0.90, '', transform=ax.transAxes,
                     color=style.DRIVER_B, fontsize=12, va='top',
                     family='monospace', zorder=6)
    gap_text = ax.text(0.02, 0.86, '', transform=ax.transAxes,
                       color=style.MUTED, fontsize=12, va='top',
                       family='monospace', zorder=6)
